<template>
  <div class="ele-body">
      ...
      >
        ...
        <template slot="rates" slot-scope="{ row }">
          <template v-for="(tag, index) in row.owner_channels">
            ...
            <br v-if="(index % 2 === 1) && (index !== row.owner_channels.length - 1)" :key="'br-'+index" />
          </template>
    ...
    <channel-edit :visible.sync="showChannelEdit" :data="current" :channel="channel" @done="reload" />
...
export default {
  name: 'AccountOwner',
  components: { UserSearch, UserEdit, ChannelEdit, UserEditBalance },
  data() {
    return {
      // 表格列配置
      columns: [
        { columnKey: 'id', type: 'index', width: 45, align: 'center', showOverflowTooltip: true },
        { prop: 'username', label: '用户名', showOverflowTooltip: true, minWidth: 90, slot: 'username' },
        { prop: 'balance', label: '余额', showOverflowTooltip: true, minWidth: 80, slot: 'balance' },
        { prop: 'daifu_balance', label: '代付余额', showOverflowTooltip: true, minWidth: 80, slot: 'daifu_balance' },
        { prop: 'pid', label: '上级', showOverflowTooltip: true, minWidth: 80, slot: 'shangji' },
        { label: '费率', showOverflowTooltip: true, minWidth: 210, slot: 'rates' },
        {
          prop: 'created_at', label: '创建时间', sortable: 'custom', showOverflowTooltip: true, minWidth: 80,
          formatter: (row, column, cellValue) => this.$util.toDateString(cellValue, 'MM-dd HH:mm:ss')
        },
        { prop: 'status', label: '状态', align: 'center', width: 120, resizable: false, slot: 'status' },
        { columnKey: 'action', label: '操作', width: 220, align: 'center', resizable: false, slot: 'action' }
      ],
      // 状态
      current: null,
      showEdit: false,
  ...
  methods: {
    ...
    handleClose(row, tag) {
      this.deleteChannel(row, tag);
    },
    deleteChannel(row, channel) {
      this.$confirm('确定要删除选中的通道？', '提示', { type: 'warning' })
        .then(() => {
    ...
    // 重置用户密码
    resetPsw(row) {
      this.$confirm('确定要重置此用户的密码为"123456"？', '提示', { type: 'warning' })
        .then(() => {
          const loading = this.$loading({ lock: true });
    ...
    // 重置谷歌
    resetGoogle(row) {
      this.$confirm('确定要重置此用户的谷歌为空吗？', '提示', { type: 'warning' })
        .then(() => {
          const loading = this.$loading({ lock: true });
