      ...
      <el-col v-bind="styleResponsive ? { md: 6, sm: 8 } : { span: 6 }">
        <el-card shadow="never" body-style="padding: 25px;">
          ...
          <div class="user-info-list">
            ...
            <div class="user-info-item">
              <i class="el-icon-office-building"></i>
              <span>锁定余额: {{ loginUser.balance_lock }}</span>
            </div>
          </div>
          <div class="user-info-list">
            <div class="user-info-item">
              <i class="el-icon-office-building"></i>
              <span>代付余额: {{ loginUser.daifu_balance }}</span>
            </div>
            <div class="user-info-item">
              <i class="el-icon-office-building"></i>
              <span>锁定代付余额: {{ loginUser.daifu_balance_lock }}</span>
            </div>
          </div>
          <div style="margin: 30px 0 20px 0">
            <el-divider class="ele-divider-dashed ele-divider-base" />
            ...
            <h6 class="ele-text" style="margin-bottom: 8px">已对接通道</h6>
            <div class="user-info-tags">
                <el-tag size="mini" type="info" v-for="item in loginUser.channels" :key="item.id">
                    {{item.channel.name}}-{{item.rate}}</el-tag>
            </div>
          </div>
        </el-card>
      </el-col>
          ...
          <el-tabs v-model="active" class="user-info-tabs">
            <el-tab-pane label="基本信息" name="info">
              ...
              >
                ...
                <el-form-item label="Appkey:" prop="merchant_key">
                  <el-input
                          ...
                          <template slot="append">
                              <el-button size="small" type="success" @click="showkey=false" v-if="showkey">显示</el-button>
                              <el-button size="small" type="success" @click="showkey=true" v-if="!showkey">隐藏</el-button>
                          </template>
                  </el-input>
                </el-form-item>
                ...
                <el-form-item label="后台IP白名单:">
                  <el-input
                    v-model="form.admin_secure_ips"
                    placeholder="请输入后台登陆IP白名单,多个IP地址用逗号分割,为空则不限制"
                    :rows="4"
                    type="textarea"
                  />
                </el-form-item>
                <el-form-item label="订单IP白名单:">
                  <el-input
                    v-model="form.secure_ips"
                    placeholder="请输入api发起订单IP,多个IP地址用逗号分割,为空则不限制"
                    :rows="4"
                    type="textarea"
                  />
                </el-form-item>
